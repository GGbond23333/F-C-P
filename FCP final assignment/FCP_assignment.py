...
class Network:
	...
	class Network:
		def get_mean_degree(self):
			...
			total_degrees = sum(sum(node.connections) for node in self.nodes)
			return total_degrees / len(self.nodes) if self.nodes else 0
		...
		def get_mean_clustering(self):
			...
			return sum(clustering_coefficients) / len(clustering_coefficients) if clustering_coefficients else 0
