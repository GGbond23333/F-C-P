...
class Network:
    ...
    def get_clustering(self):
        ...
        for node in self.nodes:
            ...
            mean_clustering = sum(clustering_coefficients) / len(clustering_coefficients) if clustering_coefficients else 0

        return mean_clustering

    def get_path_length(self):
        """
        This function calculates the mean shortest path length between nodes through the use of a Breadth-First-Search

        It iterates over every node within a network, using each node as the starting point of the search to determine
        the shortest path length. Following the acquisition of all shortest path lengths, it then computes the mean.

        Returns:
        - mean_path_length (float): The mean path length (rounded to 15 decimal places)
        """

        total_path_lengths = 0
        total_paths = 0

        for start_node in self.nodes:  # Initiate the breadth-first search
        ...
        for i, node in enumerate(self.nodes):
            for j in range(len(node.connections)):
                if node.connections[j] == 1 and np.random.random() < re_wire_prob:
                    ...
                    potential_new_connections = [k for k in range(N) if k != i and node.connections[k] == 0]
    ...
    def plot(self):
        ...
        self.plotNetwork(ax)  # Have plotNetwork actually draw the network
...
'''
==============================================================================================================
This section contains code for the Ising Model - Task 1
==============================================================================================================
'''
...
def calculate_agreement(population, row, col, external):
    """
        This function Calculates the change in agreement of a given cell

        Inputs:
        - population (array): A 2D array representing the grid of the cells
        - row (int): The row index of the cell for which agreement is to be calculated
        - col (int): The column index of a cell for which agreement is to be calculated
        - external (float): An external influence factor. e.g. social pressures, economic factors, etc.

        Returns:
        - change_in_agreement (float): The calculated change in agreement

        Circular Boundary Conditions:
        The modulo operator is used to implement circular boundary conditions.
        It ensures that neighbour acquisition wraps around the "edge" of the array.
            Limit Logic Testing:
            at (0,0) --> Left neighbour and Top neighbour --> -1 % 100 = 99
            at (99,99) --> Right neighbour and Bottom neighbour --> 100 % 100 = 0
    """

    # Finding the number of rows and columns from the population array
    numberRows, numberCols = population.shape

    # Finds the coordinates of the neighbours of a given position in the array
    neighbors = [((row - 1) % numberRows, col), ((row + 1) % numberRows, col), (row, (col - 1) % numberCols), (row, (col + 1) % numberCols)]
...
def ising_step(population, alpha, external):
    """
        ...
        Inputs:
        - population (array): A 2D array representing the grid of the cells
        - alpha (float): A factor that varies the random flips
        - external (float): An external influence factor. e.g. social pressures, economic factors, etc.

        Random Flips:
        The flips between -1 and 1 occur when agreement < 0 OR when the probability function is less than
        np.random.rand() which is a function that generates a random float between 0 and 1.
    """

    # Finds random cell to perform an ising step on
    numberRows, numberCols = population.shape
    row = np.random.randint(0, numberRows)
    col = np.random.randint(0, numberCols)

    # Calculates the agreement of the current cell
    agreement = calculate_agreement(population, row, col, external)

    # Decide whether a flip occurs
...
'''
==============================================================================================================
This section contains code for the Defuant Model - Task 2
==============================================================================================================
'''
...
def calculateAgreementOnNetwork(node, nodes, external):
    """
    This function calculates the change in agreement of a given node

    Inputs:
    - node (node): The node for which the change in agreement is being calculated
    - nodes (list): List of all nodes within the network
    - external (float): An external influence factor. e.g. social pressures, economic factors, etc.
...
def isingStepOnNetwork(network, alpha, external):  # Performs a single step of the model
    ...
    Inputs:
    - network (network): The network object which contains all the nodes
    - alpha (float): A factor that varies the random flips
    - external (float): An external influence factor. e.g. social pressures, economic factors, etc.
    """
    ...
    if agreement < 0 or np.random.rand() < np.exp(-agreement / alpha):  # np.random.rand() generates a random float between 0 and 1. if the probability function is less than this value, a flip occurs. If agreement < 0 a flip occurs
        node.value *= -1


def isingMainOnNetwork(network, alpha, external):
    """
    This function actually runs the model
